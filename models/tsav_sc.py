...
class VideoModel(nn.Module):
    def __init__(self, num_channels=3):
        ...
        self.r2plus1d = se_r2plus1d(attention = "SELayerC",pretrained=False)
...
class TwoStreamAuralVisualSelfCure(nn.Module):
    def __init__(self, num_channels=4, audio_pretrained=False, task='EX'):
        super(TwoStreamAuralVisualSelfCure, self).__init__()
        self.audio_model = AudioModel(pretrained=audio_pretrained)
        self.video_model = VideoModel(num_channels=num_channels)
        self.task = task
        fc_in_dim = self.audio_model.resnet.fc._modules['1'].in_features + self.video_model.r2plus1d.fc._modules['1'].in_features
        self.fc_au = nn.Sequential(nn.Dropout(0),nn.Linear(in_features=fc_in_dim,out_features=12))
        self.fc_ex = nn.Sequential(nn.Dropout(0),nn.Linear(in_features=fc_in_dim,out_features=8))
        self.fc_va = nn.Sequential(nn.Dropout(0),nn.Linear(in_features=fc_in_dim,out_features=2))
        
        self.alpha_au = nn.Sequential(nn.Linear(fc_in_dim, 1),nn.Sigmoid())
        self.alpha_ex = nn.Sequential(nn.Linear(fc_in_dim, 1),nn.Sigmoid())
        self.alpha_va = nn.Sequential(nn.Linear(fc_in_dim, 1),nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        #attention_weights_au = self.alpha(features)
        attention_weights_ex = self.alpha_ex(features)
        #attention_weights_va = self.alpha(features)
        #attention_weights = [attention_weights_au,attention_weights_ex,attention_weights_va]
        #out = torch.cat((self.fc_au(features)*attention_weights_au,self.fc_ex(features)*attention_weights_ex,self.fc_va(features)*attention_weights_va),1)
        out = torch.cat((self.fc_au(features),self.fc_ex(features)*attention_weights_ex,self.fc_va(features)),1)
        return out,attention_weights_ex
    ...
    def get_rank_regularization_loss(self,attention_weights,beta = 0.7,margin = 0.15):
        # Rank Regularization
        ...
        if tops>0:
            ...
            diff  = low_mean - high_mean + margin
            if diff > 0:
                RR_loss = diff
            else:
                RR_loss = 0.0
    ...
    def get_mt_mt_sc_loss(self,y_pred, att, y_true, ema_pred, ema_att, consistency_criterion):  #mean-teacher multi-task loss
        device = y_pred.device
        supervised_loss_ex = self.get_ex_loss(y_pred,y_true['EX'])
        supervised_loss_au = self.get_au_loss(y_pred, y_true['AU'])
        supervised_loss_va = self.get_va_loss(y_pred, y_true['VA'])
        ...
        supervised_ex_RR_loss = self.get_rank_regularization_loss(att)
        consistency_ex_RR_loss = self.get_rank_regularization_loss(ema_att)
        ema_pred = torch.tensor(ema_pred.detach().cpu().numpy()).to(device)
        if len(invalid_ex_indices) > 0:
            invalid_ex_indices = torch.tensor(invalid_ex_indices.flatten()).to(device)
            y_pred_ex = torch.index_select(y_pred[:, 12:20], 0, invalid_ex_indices)
        ...
        return [supervised_loss_ex,supervised_loss_au,supervised_loss_va,supervised_ex_RR_loss],[consistency_loss_ex,consistency_loss_au,consistency_loss_va,consistency_ex_RR_loss]